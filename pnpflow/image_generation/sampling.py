...
def _sde_euler_maruyama_sampler(model, sde, shape, inverse_scaler, device):
    batch_size, C, H, W = shape
    # 1) initialisation à t=1 par un bruit gaussien
    # 1) initialisation à t=1 par un bruit gaussien
    #    -> on utilise `noise_scale` (pas `init_noise_scale`)
    x = torch.randn(shape, device=device) * sde.noise_scale
    nfe = 0

    # 2) discrétisation uniforme du temps de T=1→0 en sample_N étapes
    T = torch.linspace(1.0, 0.0, sde.sample_N, device=device)
    dt = (T[1] - T[0]).item()
    model_fn = mutils.get_model_fn(model, train=False)
    for t in T:
        t_vec = torch.full((batch_size,), t, device=device)
        # – calcul de la “vitesse” par votre modèle
        v = model(x, t_vec).real
        nfe += 1
        # – pas d’Euler–Maruyama : x ← x + v·dt + σ·√|dt|·ξ
        noise_term = torch.randn_like(x) * (sde.sigma_t(t) * (dt**0.5))
        x = x + v * dt + noise_term

    # 3) remise à l’échelle dans [0,1] ou [-1,1]
    x = inverse_scaler(x)
    return x, nfe
