...
def load_cfg_from_cfg_file(file: str):
    ...
    for key, value in cfg_from_file.items():
        if isinstance(value, dict):
            # section normale
            for k, v in value.items():
                cfg[k] = v
        else:
            # section scalaire (ex. root: "…")
            cfg[key] = value
...
def define_model(args):
    if args.model in ("ot", "gradient_step","fm_indep"):
        # on utilise l'architecture U-Net issue du repo DDPM
        model = DDPMUnet(
            dim=args.dim,                             # ex. 128
            dim_mults=tuple(args.dim_mults),          # ex. (1,1,2,2,4,4)
            channels=args.num_channels,                # 3 pour RGB
            dropout=args.dropout,                      # ex. 0.0
...
def save_samples(samples, train_samples, path, args):
    ...
    # Trouve le plus grand diviseur 'cols' ≤ √n, puis rows = n/cols
    for d in range(int(math.sqrt(batch_samples_size)), 0, -1):
        if batch_samples_size % d == 0:
            cols = d
            break
    rows = batch_samples_size // cols
    fig, ax = plt.subplots(rows, 2 * cols, figsize=(20, 20))
    # — Générations du modèle (à gauche)
    for k in range(batch_samples_size):
        i = k // cols
        j = k % cols
        if args.num_channels == 1:
            ax[i, j].imshow(samples[k].squeeze(-1), cmap='gray', vmin=0, vmax=1)
        else:
            ax[i, j].imshow(samples[k])
    # — Échantillons d’entraînement (à droite)
    n_train = min(train_samples.shape[0], batch_samples_size)
    for k in range(n_train):
        i = k // cols
        j = k % cols
        col_idx = j + cols
        if args.num_channels == 1:
            ax[i, col_idx].imshow(train_samples[k].squeeze(-1),
                                   cmap='gray', vmin=0, vmax=1)
        else:
            ax[i, col_idx].imshow(train_samples[k])
